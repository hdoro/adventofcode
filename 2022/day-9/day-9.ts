...
function moveKnot(knot: Knot, direction: string, knotToCompare?: Knot): Knot {
  ...
  const diffX = knotToCompare[0] - posX
  const diffY = knotToCompare[1] - posY
  ...
  return [posX + sign(diffX), posY + sign(diffY)]
}
...
function getFinalState(
  ...
  return input
    ...
    .reduce(
      (prevState, curMove) => {
        const move = executeMove(curMove, prevState.knots)
        return {
          knots: move,
          tailHistory: [move.slice(-1)[0], ...prevState.tailHistory],
        }
      },
      { knots: getInitialState(ropeLength), tailHistory: [] } as {
        knots: Knot[]
        tailHistory: Position[]
      },
    )
...
export function getTailCells(input: string, ropeLength: number) {
  ...
  return new Set(finalState.tailHistory.map((placement) => placement.join(':')))
    .size
}
